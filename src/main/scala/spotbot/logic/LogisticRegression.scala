...
object LogisticRegression {
  ...
  def hTheta(x: Vector[Double]): Double = {

    sigmoid(x.zip(theta).map(x => x._1 * x._2).sum)

  }

  def costFunction(x: Vector[Double], y: Double): Double = {

    -y*math.log(hTheta(x))-(1-y)*math.log(hTheta(x))
